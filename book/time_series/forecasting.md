...
To be able to estimate future values of the time series (forecasting), we need to first estimate the components of the time series. Additionally, the time series needs to be *stationary*, i.e., the statistical properties do not change over time ([Chapter 4.4](stationary)). An example of a stationary time series is white noise, and the AR(1) process with $|\phi| < 1$. To turn the time series stationary, we can detrend it and use the residuals from a least-squares fit.

After ensuring the time series is stationary, we can perform a stochastic analysis. In some cases the noise is not white but auto-correlated, meaning that the observations at different time lags depend on each other. This type of noise is referred to as colored noise. The dependence between the time lags can be analysed using the autocovariance function ([Chapter 4.5](ACF)). Based on the ACF, the noise process can be modelled using the Autoregressive model ([Chapter 4.6](AR)). The order of the AR model depends on the data. Apart from the AR process, many other noise processes exist, such as ARMA and ARIMA, but these are outside the scope of MUDE. In this course, we focus on AR with order 1, AR(1). Based on the AR(1) process, to model the noise in the time series we need to estimate parameter $\phi$.

Once the signal-of-interest and the noise process are characterized, we can forecast future values of the time series.
...
3. Predict the signal-of-interest: $\hat{Y}_{signal}=\mathrm{A}_p\hat{X}$, where $\mathrm{A}_p$ is the design matrix describing the functional relationship between the future values $Y_p$ and $\mathrm{x}$. A mathematical proof can be found in Chapter 11 of [Surveying and Mapping (2022)](https://doi.org/10.5074/T.2021.007) by Christian C. J. M. Tiberius, et al. (2022).
