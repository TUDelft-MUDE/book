...
To be able to estimate future values of the time series (forecasting), we need to first estimate the components of the time series. Additionally, the time series needs to be *stationary*, i.e., the statistical properties do not change over time ([Chapter 4.4](stationary)). We considered a white noise time series, and the AR(1) process with $|\phi| < 1$. To turn a time series into a stationary one, we can detrend it and use the residuals from a least-squares fit for stochastic noise modelling.
