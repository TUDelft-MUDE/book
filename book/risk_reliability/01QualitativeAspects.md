...
where $R_A$ is the risk of the event A, $p_A$ is the probability of the event A happening, and $C_A$ are the consequences derived from the event A happening. Event A could be, for instance, the occurrence of an accident in a bridge that leads to stopping the traffic.
