...
We need to use a more complex model to answer to this more complex question. In this case, the Finite Element Method (More on that in Q2) would be an appropriate one.
...
* What is the concentration of a contaminant in a given location of my reservoir?
...
We can also do a structured walk-through/one-step analysis. i.e., explain your steps to a non-expert person (or a rubber duckie!). This helps you to go through the modelling steps again.
...
We can try running the model for extreme cases. Checking asymptotic convergence of simulations or verifying a good representation of floating-point numbers are a few examples of performing this step.
...
Once we have our measurable parameters defined, the key phase of the validation process takes place: the comparison of the results of our model with the reference value. As a result, we will be in two possible scenarios:
