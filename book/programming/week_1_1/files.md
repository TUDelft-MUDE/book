...
MUDE has a repetitive weekly workflow and a _lot_ of different technical topics. Because of this, it is important to keep our files well organized. This page gives you the background information to do this efficiently. Your weekly assignments will also guide you through the process of setting this up in a gradual way.
...
The _file system_ on a computer is simply a way to organize digital documents. Because it is easily customizable, it can be configured in many ways, enabling the computer user to optimize it for their own purposes. For MUDE, it is useful to define a few key concepts:
...
```{tip}
Some of the content on this page describes the software Git and a related concept of _repository._ If you are unfamiliar with these concepts, you can ignore them for now and consider a _repository_ to be equivalent to a _directory_ or _sub-directory._
```

## Your Local File System

You will be working with a _lot_ of different files and file types in MUDE: `csv`, `pdf`, `ipynb`, `txt`, `md` just to name a few. Remember, there are 16 weeks of class in MUDE---it is good practice to get in the habit of storing files in an organized way, not to mention make it easier to study for the exams. We recommend creating a folder `MUDE` to collect all files associated with the module, then set up a new folder for each week and each project as your **working directory** for each of those activities. This is where you will download files and edit them when working on them. 
...
├── MUDE                           <-- a root directory for MUDE projects
    ...
    ├── Project_2                  <-- another working directory
        ├── ...
    ├── Project_3                  <-- each of these should be its own repository
        ├── ...
    ├── Project_4
        ├── ...
```
...
```{figure} https://github.com/TUDelft-MUDE/source-files/raw/main/file/working_dir.svg
---
width: 40%
name: working_dir
---
