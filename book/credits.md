...
>
> {numref}`openstax_fig_01` is reproduced in this chapter of the book from {cite:t}`openstax_calculus` without modification and is _not_ included under the CC BY license of this book. The source content is provided with a CC BY NC SA license and can be accessed for free at [](https://openstax.org/books/calculus-volume-2/pages/1-introduction).
